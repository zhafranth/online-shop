...
export function CartItem({ item, index, onUpdateQty, onRemove }: CartItemProps) {
  return (
    <div className="flex gap-5 py-6 border-b border-site-border items-start">
      <PlaceholderImage label={item.label} className="w-[100px] h-[120px] shrink-0" />
      <div className="flex-1">
        <div className="flex justify-between items-start gap-4">
          <div>
            <div className="text-[11px] text-site-gray mb-0.5">{item.category}</div>
            <Link href={`/product/${item.id}`} className="font-serif text-lg mb-1.5 cursor-pointer no-underline text-site-text hover:text-navy block">{item.name}</Link>
        ...
        <div className="flex justify-between items-center mt-4">
          ...
          <div className="text-right">
            <div className="font-serif text-lg font-semibold text-navy">{formatPrice(item.price * item.qty)}</div>
