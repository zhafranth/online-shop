          ...
          {newItems.map((p) => (
            <Link key={p.id} href={`/product/${p.id}`} className="group cursor-pointer block no-underline text-site-text">
              <div className="overflow-hidden relative">
                <PlaceholderImage src={p.image} alt={p.name} label={p.label} sizes="(max-width: 768px) 50vw, 25vw" className="w-full h-[220px] sm:h-[260px] md:h-[280px] transition-transform duration-400 group-hover:scale-[1.04]" />
