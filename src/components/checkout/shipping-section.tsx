        ...
        <div className="p-4 sm:p-5 border-b border-site-border bg-site-white">
          <div className="flex flex-col gap-2.5">
            {SHIPPING_OPTIONS.map((opt) => (
              <label key={opt.id} className={`flex items-center gap-3 sm:gap-3.5 p-3 sm:p-3.5 border-[1.5px] cursor-pointer ${selected === opt.id ? "border-navy bg-cream" : "border-site-border bg-white"}`}>
                <input type="radio" name="shipping" value={opt.id} checked={selected === opt.id} onChange={() => onSelect(opt.id)} className="accent-[#1a2744] shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-sm">{opt.label}</div>
                  <div className="text-xs text-site-gray">{opt.description}</div>
                </div>
                <div className={`font-semibold text-sm shrink-0 ${isFreeShipping ? "text-gold" : "text-navy"}`}>{isFreeShipping ? "FREE" : formatPrice(opt.price)}</div>
              </label>
            ))}
          </div>
