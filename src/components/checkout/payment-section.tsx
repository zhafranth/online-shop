        ...
        <div className="p-5 border-b border-site-border bg-site-white">
          <div className="grid grid-cols-2 gap-2.5">
            {PAYMENT_OPTIONS.map((opt) => (
              <label key={opt.id} className={`flex items-start gap-2.5 p-3.5 border-[1.5px] cursor-pointer ${selected === opt.id ? "border-navy bg-cream" : "border-site-border bg-white"}`}>
          ...
          <Button variant="primary" className="mt-4" onClick={onSave}>Konfirmasi Pembayaran</Button>
        </div>
      )}
    </div>
  );
